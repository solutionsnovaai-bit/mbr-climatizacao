  ...
  return (
    <button
      ...
      className={`relative inline-flex h-11 items-center gap-2 rounded-full px-4 text-[0.92rem] font-semibold transition-all duration-300 ${
        on
          ? 'bg-navy text-white shadow-[0_12px_26px_-14px_rgba(1,38,90,0.9)]'
    ...
    >
      <AnimatePresence initial={false}>
        {on && (
          ...
          >
            <Check className="size-4 text-azul-glow" strokeWidth={3} />
          </motion.span>
        )}
      </AnimatePresence>
      {children}
...
export function Diagnostic() {
  ...
  const message = useMemo(() => {
    const lines = ['Olá, MBR! Vim pelo site.', `Equipamento: ${equip}.`]
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-5 sm:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-16">
        <div>
          <h2 className="wide text-[2.1rem] font-extrabold leading-[1.02] tracking-[-0.02em] text-navy sm:text-[3rem]">
            O que o seu equipamento está fazendo?
          </h2>
          ...
          <fieldset className="mt-10">
            <legend className="text-[0.95rem] font-bold text-navy">Qual equipamento?</legend>
          ...
          <label className="mt-8 block max-w-md">
            ...
            <input
              ...
              maxLength={60}
              className="mt-3 h-12 w-full rounded-2xl bg-white/80 px-4 text-[0.98rem] text-navy ring-1 ring-navy/10 placeholder:text-tinta/40 focus:outline-none focus:ring-2 focus:ring-azul"
            />
          </label>
        </div>
        ...
        <div className="lg:sticky lg:top-28 lg:self-start">
          <div className="glass glass-strong relative overflow-hidden rounded-[32px] p-5 sm:p-7">
            ...
            <div
              ...
              style={{ background: 'linear-gradient(160deg, rgba(201,225,244,0.7), rgba(234,243,250,0.5))' }}
            >
              <motion.div
                layout
                className="ml-auto max-w-[92%] whitespace-pre-line rounded-[20px] rounded-br-md bg-gradient-to-br from-azul to-navy px-4 py-3 text-[0.95rem] leading-relaxed text-white shadow-[0_16px_30px_-18px_rgba(1,38,90,0.9)]"
